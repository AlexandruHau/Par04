...
void Par04RunAction::BeginOfRunAction(const G4Run*)
{
  ...
  // Create the NTuple now
  // analysisManager->CreateNtuple("FullSims energy", "energy hits");
  // analysisManager->CreateNtupleDColumn("FullSims_energy");
  analysisManager->CreateNtuple("Infered energy", "energy hits");
  analysisManager->CreateNtupleDColumn("Infered_energy");
  analysisManager->CreateNtupleDColumn("x_coordinate");
  analysisManager->CreateNtupleDColumn("y_coordinate");
  analysisManager->CreateNtupleDColumn("z_coordinate");
  analysisManager->FinishNtuple();

  // Create the file as well
  // G4String fileName = "FullSims";
  G4String fileName = "ONNXInference";
  ...
  // Need to create separate histogram for the energy calculated through
  // the ONNX Inference framework
  // analysisManager->CreateH1("Inferred energy", "Inferred Energy from Fast Simulations", 1024, 0, 1*MeV);
  ...
  // Create separate ntuple for the energy calculated through the
  // ONNX Inference framework
  analysisManager->CreateNtupleDColumn("Inferred_energy");
  analysisManager->FinishNtuple();
