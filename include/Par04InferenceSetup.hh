...
class Par04InferenceSetup
{
 public:
  ...
  void GetPositions(std::vector<G4ThreeVector>& aDepositsPositions, G4ThreeVector aParticlePosition,
                    G4ThreeVector aParticleDirection, std::vector<G4double>& aEnergies,
 ...
 private:
  ...
  /// Need the 3DGAN constant for division
  G4int f3DGANConstant = 100;
  /// Need the rescale factor from G4 to 3DGAN
  G4int fG4rescale = 1000;
  /// Need the opposite of the power raisal
  G4float fPowerStable = (1/0.85);
  /// Need the last division as well
  G4int fDivideStable = 50;
};
